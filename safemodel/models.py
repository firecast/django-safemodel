...
from .config import SafePolicy
from .queryset import SafeModelQueryset


class SafeModel(models.Model):
    _safe_policy = SafePolicy.SOFT_DELETE
    ...
    def delete(self, force_policy=None, deleted_on=None, **kwargs):
        ...
        if current_policy == SafePolicy.HARD_DELETE:
            super(SafeModel, self).delete(**kwargs)
        elif current_policy == SafePolicy.HARD_DELETE_NOCASCADE:
            if self.can_hard_delete():
                super(SafeModel, self).delete(**kwargs)
            else:
                self.delete(force_policy=SafePolicy.SOFT_DELETE)
        elif current_policy == SafePolicy.SOFT_DELETE:
            self.deleted = True
            self.deleted_on = timezone.now()
            super(SafeModel, self).save(**kwargs)
        elif current_policy == SafePolicy.SOFT_DELETE_CASCADE:
            ...
            self.delete(force_policy=SafePolicy.SOFT_DELETE, deleted_on=deleted_timestamp, **kwargs)
        elif current_policy == SafePolicy.NO_DELETE:
            return
    ...
    def undelete(self, force_policy=None, deleted_on=None, **kwargs):
        ...
        if current_policy == SafePolicy.SOFT_DELETE_CASCADE:
            for related_field in self.related_fields():
